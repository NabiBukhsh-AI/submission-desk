"""A deterministic stand-in for a model, so the harness can be run by anybody.

What this is for, stated plainly because it is the easiest thing in the whole
repository to misread.

Arm C measures the *pipeline*: span validation, the rule engine, the coverage
gate, abstention, the integrity path. Those are deterministic, and testing them
needs a model-shaped thing that answers the same way every time. This is that
thing. It finds sentences in the document that match a criterion's own example
phrases and quotes them verbatim.
...
into the right band — not whether a language model can read a CV. The report says
so at the top of every run that used it, because a reader who mistook these for
...
Run against a real provider by setting MODEL_PROVIDER, and the same harness
measures the model instead.
